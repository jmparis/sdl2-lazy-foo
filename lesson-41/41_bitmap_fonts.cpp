...
//  Using SDL, SDL_image, SDL_ttf, standard IO, math, strings, and
//	string streams
#include <SDL.h>
#include <SDL_image.h>
#include <stdio.h>
...
bool LTexture::loadFromFile( std::string path )
{
	...
	else
	{
		...
		{
			printf(
                "Unable to convert loaded surface to display format! %s\n"  ,
                SDL_GetError()
            );
		}
		else
			...
			for ( int cols = 0; cols < 16; ++cols )
			{
				...
				for ( int pColW = cellW - 1; pColW >= 0; --pColW )
				{
					...
					for ( int pRowW = 0; pRowW < cellH; ++pRowW )
					{
						...
						{
							//  Set the width
							mChars[ currentChar ].w =
                                ( pX - mChars[ currentChar ].x ) + 1;
